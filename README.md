...
The script can be executed with the optional argument "-c" (alternatively "--color"), which changes the output graph from black/white into color. The argument "-bw" (resp. "--blackwhite") forces the black/white mode, which is currently set as standard.
